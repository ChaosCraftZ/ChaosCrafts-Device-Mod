...
public class GeometryDashApp implements IApp {
    ...
    private final int cubeSize = playerSize;
    private volatile String currentMode = "Cube";
    private volatile int activeSpeedMultiplier = 1;
    ...
    private float takeoffBaseAngle = 0f;
    private float airborneDistanceAccum = 0f;
    private boolean rotatingInAir = false;
    ...
    private void gameUpdateLoop() {
        try {
            if (state == GameState.PLAYING && gameStarted.get()) {
                ...
                float dt = Math.max(1f, deltaMillis) / 16.0f;
    ...
    private void updateGame(float deltaTime) {
        ...
        if (obstacles.isEmpty() || obstacles.get(obstacles.size()-1).x < 240 + rng.nextInt(160)) {
            ...
            if (typeRoll < 50) {
                int cubesVert = 1 + rng.nextInt(2);
                int cubesHor = 1 + rng.nextInt(3);
        ...
        for (Obstacle o : obstacles) {
            ...
            if (o.type == ObstacleType.SPIKE) {
                ...
                float hbTop = (o.height / 2f) - (small / 2f);
        ...
        final float epsilonGround = 0.0005f;
        ...
        if (rotatingInAir && !onGround) {
            airborneDistanceAccum += horizontalSpeed * deltaTime;
            float rotations = (airborneDistanceAccum / playerSize) * 90f;
    ...
    private void drawSpike(GuiGraphics g, int x, int yTop, int size) {
        int topColor = 0xFFFFE6B3;
        int midColor = 0xFFFFC066;
        int baseColor = 0xFFFF8C1A;
