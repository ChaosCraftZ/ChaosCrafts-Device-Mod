...
public class NotepadApp implements IApp {
    ...
    private static final Logger LOGGER = LogManager.getLogger(NotepadApp.class);
    ...
    private void initializeComponents() {
        ...
        this.textArea = new MultiLineEditor(this.font != null ? this.font : mc.font);
        this.textArea.setResponder(this::onTextChanged);

        this.saveFileNameInput = new EditBox(this.font != null ? this.font : mc.font, 0, 0, 200, 16, Component.empty());
        this.saveFileNameInput.setMaxLength(255);
        this.filePathInput = new EditBox(this.font != null ? this.font : mc.font, 0, 0, 300, 16, Component.empty());
        this.filePathInput.setMaxLength(500);

        this.openFileNameInput = new EditBox(this.font != null ? this.font : mc.font, 0, 0, 200, 16, Component.empty());
        this.openFileNameInput.setMaxLength(255);
        this.openFilePathInput = new EditBox(this.font != null ? this.font : mc.font, 0, 0, 300, 16, Component.empty());
    ...
    private void openFile(File file) {
        asyncRuntime.submitIoThenClient(() -> Files.readString(file.toPath()), content -> {
            ...
            asyncRuntime.runOnClientThread(() -> showStatus("Error opening: " + ex.getMessage()));
            return null;
        });
    }
    ...
    private void asyncSaveDocument(File file) {
        asyncRuntime.submitIoThenClient(() -> {
            ...
            asyncRuntime.runOnClientThread(() -> showStatus("Error saving: " + ex.getMessage()));
            return null;
        });
    }
    ...
    private void undo() {
        ...
        String cur = undoStack.poll();
        if (cur == null) return;
        redoStack.push(cur);
        String prev = undoStack.peek();
        if (prev != null) {
            asyncRuntime.runOnClientThread(() -> textArea.setValue(prev));
        }
    }

    private void redo() {
        if (redoStack.isEmpty()) return;
        String n = redoStack.pop();
        pushUndoState();
        asyncRuntime.runOnClientThread(() -> textArea.setValue(n));
    }

    // recent files
    private void addRecent(File f) {
        try {
            String p = f.getAbsolutePath();
            recentFiles.remove(p);
            recentFiles.addFirst(p);
            while (recentFiles.size() > 20) recentFiles.removeLast();
            saveRecentFiles();
        } catch (Exception ex) {
            LOGGER.warn("Failed to add recent file: {}", f, ex);
        }
    }
    ...
    private void loadRecentFiles() {
        ...
        try {
            if (recentFilesFile.exists()) {
                List<String> lines = Files.readAllLines(recentFilesFile.toPath());
                for (String l : lines) if (!l.isBlank()) recentFiles.add(l);
            }
        } catch (IOException ex) {
            LOGGER.warn("Failed to load recent files from {}", recentFilesFile, ex);
        }
    }

    private void saveRecentFiles() {
        try {
            if (recentFilesFile.getParentFile() != null) recentFilesFile.getParentFile().mkdirs();
            try (Writer w = new FileWriter(recentFilesFile, false)) { for (String s : recentFiles) w.write(s + "\n"); }
        } catch (IOException ex) {
            LOGGER.warn("Failed to save recent files to {}", recentFilesFile, ex);
        }
    }
    ...
    public void renderContent(GuiGraphics guiGraphics, PoseStack poseStack, DraggableWindow window, int mouseRelX, int mouseRelY, float partialTick) {
        ...
        if (showMenuBar) {
            ...
            guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("File"), cx + 8, cy + 6, 0xFFFFFFFF, false);
            ...
            guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save"), saveBtnX + 10, saveBtnY + 4, 0xFFFFFFFF, false);
            // File menu dropdown
            if (fileMenuOpen) {
                ...
                for (String item : fileMenuItems) {
                    if (item.equals("-")) {
                        ...
                        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal(item), mx + 8, my + idx * fileMenuItemHeight + 4, 0xFFFFFFFF, false);
                    }
                    idx++;
                }
                // Recent files
                if (!recentFiles.isEmpty()) {
                    guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Recent files:"), mx + 8, my + idx * fileMenuItemHeight + 2, 0xFFAAAAAA, false);
                    idx++;
                    int rIdx = 0;
                    for (String rf : recentFiles) {
                        ...
                        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal(shortName), mx + 16, my + idx * fileMenuItemHeight + 4, 0xFFFFFFFF, false);
                        idx++;
                        rIdx++;
                        if (rIdx >= 5) break;
                    }
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal(name + (isModified.get() ? " *" : "") + "  |  Ln " + lc[0] + ", Col " + lc[1]), cx + 5, statusY + 5, 0xFFFFFFFF, false);
        if (System.currentTimeMillis() - statusMessageTime < 3000) guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal(statusMessage), cx + cw - 10 - (this.font != null ? this.font : Minecraft.getInstance().font).width(statusMessage), statusY + 5, 0xFFFFFFFF, false);
    ...
    private void renderSaveDialog(GuiGraphics guiGraphics, int cx, int cy, int cw, int ch) {
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save Document"), x + 8, y + 6, 0xFFFFFFFF, false);

        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("File name:"), x + 8, y + 34, 0xFFFFFFFF, false);
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save in:"), x + 8, y + 62, 0xFFFFFFFF, false);
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Cancel"), x + w - 158, y + h - 32, 0xFFFFFFFF, false);
        guiGraphics.fill(x + w - 100, y + h - 36, x + w - 40, y + h - 12, 0xFF4CAF50);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save"), x + w - 90, y + h - 32, 0xFFFFFFFF, false);
    }
    ...
    private void renderOpenDialog(GuiGraphics guiGraphics, int cx, int cy, int cw, int ch) {
        ...
        guiGraphics.fill(x, y, x + w, y + h, 0xFF2B2B2B);
        guiGraphics.fill(x, y, x + w, y + 20, 0xFF4C7BD1);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Open Document"), x + 8, y + 6, 0xFFFFFFFF, false);
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Open from:"), x + 8, y + 62, 0xFFFFFFFF, false);
        ...
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Cancel"), x + w - 158, y + h - 32, 0xFFFFFFFF, false);
        guiGraphics.fill(x + w - 100, y + h - 36, x + w - 40, y + h - 12, 0xFF4CAF50);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Open"), x + w - 90, y + h - 32, 0xFFFFFFFF, false);
    }
    ...
    private void renderSaveOnExitDialog(GuiGraphics guiGraphics, int cx, int cy, int cw, int ch) {
        ...
        int y = cy + (ch - h) / 2;
        guiGraphics.fill(x, y, x + w, y + h, 0xFF2B2B2B);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save changes before exiting?"), x + 16, y + 18, 0xFFFFFFFF, false);
        guiGraphics.fill(x + 20, y + h - 36, x + 100, y + h - 12, 0xFF4CAF50);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Save"), x + 36, y + h - 32, 0xFFFFFFFF, false);
        guiGraphics.fill(x + 120, y + h - 36, x + 200, y + h - 12, 0xFF888888);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Don't Save"), x + 126, y + h - 32, 0xFFFFFFFF, false);
        guiGraphics.fill(x + 220, y + h - 36, x + 300, y + h - 12, 0xFF888888);
        guiGraphics.drawString(this.font != null ? this.font : Minecraft.getInstance().font, Component.literal("Cancel"), x + 236, y + h - 32, 0xFFFFFFFF, false);
    }
