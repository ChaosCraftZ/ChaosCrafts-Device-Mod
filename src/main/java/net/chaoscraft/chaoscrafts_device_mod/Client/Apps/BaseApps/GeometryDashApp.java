...
public class GeometryDashApp implements IApp {
    ...
    public void onOpen(DraggableWindow window) {
        ...
        updateTask = asyncRuntime.scheduleAtFixedRate(this::gameUpdateLoop, 16, 16, TimeUnit.MILLISECONDS);
    }
    ...
    private void gameUpdateLoop() {
        if (state == GameState.PLAYING && gameStarted.get()) {
            ...
            asyncRuntime.runOnClientThread(() -> updateGame(dt));
        }
    }
    ...
    public void renderContent(GuiGraphics guiGraphics, PoseStack poseStack, DraggableWindow window, int mouseRelX, int mouseRelY, float partialTick) {
        ...
        synchronized (orbs) {
            for (Orb ob : orbs) {
                int orbColor;
                switch (ob.type) {
                    case YELLOW: orbColor = 0xFFFFFF00; break; // Yellow coin orb
                    case PINK: orbColor = 0xFFFF66FF; break;   // Pink pad orb
                    case RED: orbColor = 0xFFFF4444; break;    // Red portal orb
                    case GREEN: orbColor = 0xFF44FF44; break;  // Green speed orb
                    case BLUE: orbColor = 0xFF4444FF; break;   // Blue gravity orb
                    default: orbColor = 0xFFFFFF66; break;
                }
                guiGraphics.fill(cx + ob.x, groundY - ob.y - 8, cx + ob.x + 12, groundY - ob.y + 4, orbColor);
                // Add a glowing effect
                guiGraphics.fill(cx + ob.x + 2, groundY - ob.y - 6, cx + ob.x + 10, groundY - ob.y + 2, orbColor | 0x88000000);
            }
        }
    ...
    private void updateGame(float deltaTime) {
        ...
        // spawn logic (mix blocks, spikes, pads, portals, orbs)
        if (obstacles.isEmpty() || obstacles.get(obstacles.size()-1).x < 240 + rng.nextInt(160)) {
            ...
            if (typeRoll < 30) {
                ...
                // Orb: collectible with different types and effects (25% chance)
                synchronized (orbs) {
                    OrbType[] types = OrbType.values();
                    OrbType orbType = types[rng.nextInt(types.length)];
                    int orbY;
                    
                    // Strategic orb placement based on type
                    switch (orbType) {
                        case YELLOW: // Coin orb - low placement
                            orbY = 30 + rng.nextInt(30);
                            break;
                        case PINK: // Pad orb - medium height
                            orbY = 45 + rng.nextInt(35);
                            break;
                        case RED: // Portal orb - higher placement
                            orbY = 60 + rng.nextInt(40);
                            break;
                        case GREEN: // Speed orb - varies
                            orbY = 35 + rng.nextInt(50);
                            break;
                        case BLUE: // Reverse gravity orb - high
                            orbY = 70 + rng.nextInt(30);
                            break;
                        default:
                            orbY = 40 + rng.nextInt(60);
                    }
                    
                    orbs.add(new Orb(startX, orbY, orbType));
                }
            }
        }
        ...
        synchronized (orbs) {
            ...
            while (oit.hasNext()) {
                ...
                if (ob.x < 60 + playerSize && ob.x + 12 > 60) {
                    if (playerY <= ob.y + 8) {
                        // collect orb based on type
                        switch (ob.type) {
                            case YELLOW: // Coin orb - points only
                                score.addAndGet(25);
                                break;
                            case PINK: // Pad orb - small jump boost
                                playerVel = 12f;
                                score.addAndGet(25);
                                break;
                            case RED: // Portal orb - medium jump + speed boost
                                playerVel = 16f;
                                activeSpeedMultiplier = Math.min(4, activeSpeedMultiplier + 1);
                                score.addAndGet(50);
                                break;
                            case GREEN: // Speed orb - temporary speed increase
                                activeSpeedMultiplier = Math.min(4, activeSpeedMultiplier + 1);
                                score.addAndGet(25);
                                break;
                            case BLUE: // Reverse gravity orb - big jump
                                playerVel = 18f;
                                score.addAndGet(50);
                                break;
                        }
    ...
    private static class Orb { 
        int x; int y; OrbType type; 
        Orb(int x, int y, OrbType type) { this.x = x; this.y = y; this.type = type; }
    }
    private enum OrbType { YELLOW, PINK, RED, GREEN, BLUE }
}
