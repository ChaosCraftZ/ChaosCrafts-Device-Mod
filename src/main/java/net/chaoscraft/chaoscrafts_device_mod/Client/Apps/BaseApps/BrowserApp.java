...
public class BrowserApp implements IApp {
    ...
    @Override public void onOpen(DraggableWindow window) { this.font = Minecraft.getInstance().font; }
    ...
    guiGraphics.drawString(font, Component.literal("Address:"), cx + 6, cy + 8, 0xFFFFFFFF, false);
    if (addressBox == null) addressBox = new EditBox(font, cx + 64, cy + 6, Math.max(80, cw - 160), 16, Component.literal("url"));
    ...
    guiGraphics.drawString(font, Component.literal("Go"), cx + cw - 82, cy + 8, 0xFFFFFFFF, false);

        // content display
    guiGraphics.drawString(font, Component.literal(content), cx + 6, cy + 40, 0xFFFFFFFF, false);
    }
    ...
    private void goTo(String url) {
        ...
        asyncRuntime.submitIoThenClient(() -> {
            ...
            final String errorMsg = "Failed to load: " + ex.getClass().getSimpleName() + " - " + ex.getMessage();
            asyncRuntime.runOnClientThread(() -> content = errorMsg);
            return null;
        });
    }
}